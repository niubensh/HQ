...
public class VoiceDBHelper extends SQLiteOpenHelper {
    ...
    private final static String DB_NAME = "voice.db";
    private final static String TABLE_NAME = "voice";
    ...
    private final static String CREATE_TBL = "create table voice(id integer,name text, voicefile text ,isread int)";
    private SQLiteDatabase db;
    //SQLiteOpenHelper子类必须要的一个构造函数
    public VoiceDBHelper(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
        //必须通过super 调用父类的构造函数
        super(context, name, factory, version);
    }
    //数据库的构造函数，传递三个参数的
    public VoiceDBHelper(Context context, String name, int version){
...
//    }
    ...
    public int queryforisread( ) {
        SQLiteDatabase db = getReadableDatabase();
        //获取Cursor
        Cursor c = db.query(TABLE_NAME, null,"isread=?", new String[]{"0"}, null, null, null, null);
        int count=0;
        while(c.moveToNext()){
            count++;
        }
